...
Glosses come from the Wiktionary **Action API**, which takes up to 50 titles per request, so a whole subtitle cue is one upstream call; `glossary.py` parses the returned wikitext. Both upstreams rate-limit per IP by request count, so each has a cooldown: once refused, it is skipped entirely until the window passes rather than retried. Wiktionary results (including "no entry") persist in `backend/.gloss-cache.json` — git-ignored, safe to delete, rebuilt on demand — so a restart does not re-spend the budget.
