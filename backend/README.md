...
Both upstreams rate-limit per IP, so each has a cooldown: once refused, it is skipped entirely until the window passes rather than retried. Wiktionary results (including "no entry") persist in `backend/.gloss-cache.json` — git-ignored, safe to delete, rebuilt on demand — so a restart does not re-spend the budget.
